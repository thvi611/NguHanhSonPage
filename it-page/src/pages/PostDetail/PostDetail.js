...
export default function PostDetail() {
    ...
    useEffect(() => {
        const fetchPost = async () => {
            const response = await fetch(`https://88f6-1-53-200-28.ap.ngrok.io/api/${type}/${id}`);
    ...
    const handleClick = async (e) => {
        ...
        if (type === "post") {
            ...
            try {
                await axios.post("https://88f6-1-53-200-28.ap.ngrok.io/api/comment", comment);
            } catch (err) {
                console.log(err);
            }
        }
            ...
            try {
                await axios.post("https://88f6-1-53-200-28.ap.ngrok.io/api/comment", comment_guide);
            } catch (err) {
                console.log(err);
            }
        }
    ...
    const renderImage = (image) => {
        if (image.url.includes("/images/"))
            return (
                <img src={"https://88f6-1-53-200-28.ap.ngrok.io/storage" + image.url} alt='' style={{ height: "50%", width: "100%", objectFit: "contain" }} className="m-0" />
            );
        else
            return (
    ...
    const handleDeletePost = async () => {
        try {
            await axios.delete(`https://88f6-1-53-200-28.ap.ngrok.io/api/post/${id}`);
            navigate("/");
        } catch (err) {
            console.log(err);
        }
